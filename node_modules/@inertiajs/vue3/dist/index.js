  ...
  methods: {
    ...
    addTitleElement(elements) {
      if (this.title && !elements.find((tag) => tag.startsWith("<title"))) {
        elements.push(`<title ${this.provider.preferredAttribute()}>${(0, import_lodash_es4.escape)(this.title)}</title>`);
      }
      return elements;
    },
    renderNodes(nodes) {
...
function usePoll(interval, requestOptions = {}, options = {
  ...
  const { stop, start, destroy } = import_core8.router.poll(interval, requestOptions, {
    ...options,
    autoStart: false
  });
